...
class UserService extends BaseService {
     ...
     * @param {*} ctx 
     */
    RegisterUser(user, callback, ctx) {
    ...
    add(user, callback, ctx) {
        ...
        var sql = "INSERT INTO st_user (CREATED_DATETIME,MODIFIED_DATETIME,FIRST_NAME,LAST_NAME,LOGIN,PASSWORD,DOB,MOBILE_NO,ROLE_ID,GENDER,PIC) "
            + " VALUES (NOW(),NOW(),?,?,?,?,?,?,?,?,?)";
        var params = [user.firstName, user.lastName, user.login,
        user.password, user.dob, user.mobileNo, user.roleId, user.gender,user.pic];
        super.executeSQL(sql, params, function (err, result) {
            if (err) {
                callback(err);
            } else {
